                                    ...
                                    <td>
                                        <div class="d-flex gap-1 flex-wrap">
                                            <a href="{{ route('admin.drivers.show', $driver) }}"
                                                class="btn btn-sm btn-outline-secondary" style="border-radius:8px;font-size:.75rem;">
                                                View
                                            </a>
                                            @if($approval === 'pending')
                                                <form method="POST" action="{{ route('admin.drivers.approve', $driver) }}">
                                                    @csrf
                                                    <button class="btn btn-sm btn-success" style="border-radius:8px;font-size:.75rem;">
                                                        Approve
                                                    </button>
                                                </form>
                                            @endif
                                            @if($driver->vehicle && !$driver->vehicle->is_verified)
                                                    ...
                                                    $driver->vehicle
                                                                        ...
                                                                        @csrf
                                                                        <button class="btn btn-sm btn-warning" style="border-radius:8px;font-size:.75rem;">
                                                                            Verify Vehicle
                                                                        </button>
                                                                    </form>
                                            @endif
