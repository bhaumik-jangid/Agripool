...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ route('farmer.dashboard') }}" class="nav-item-custom
          {{ request()->routeIs('farmer.dashboard') ? 'active' : '' }}">
            ...
            @php
                $pendingProposals = \App\Models\PriceProposal::whereIn(
                    'pool_id',
                    \App\Models\PoolMember::where('user_id', Auth::id())
                        ->pluck('pool_id')
                )->where('status', 'pending')
                    ->where('expires_at', '>', now())
                    ->get()
                    ->filter(fn($p) => !$p->hasVoted(Auth::id()))
                    ->count();
            @endphp

            <a href="{{ route('farmer.dashboard') }}#proposals" class="nav-item-custom">
                <span class="nav-icon">💬</span> Price Proposals
                @if($pendingProposals > 0)
                    <span class="badge-notif">{{ $pendingProposals }}</span>
                @endif
            </a>
    ...
    <div class="main-content">
        ...
        <div class="topbar">
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <span class="badge rounded-pill" style="background:var(--green-pale);color:var(--green-dark);">
                    🌾 Farmer
                </span>
            </div>
        </div>
