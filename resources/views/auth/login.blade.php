...
@section('content')
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="small text-decoration-none" style="color: #2d6a4f;">Forgot
                    password?</a>
            @endif
        </div>

        {{-- Submit --}}
        <button type="submit" class="btn w-100 py-2 fw-bold text-white" style="background-color: #2d6a4f;">
            Sign In
        </button>
    ...
    <div class="text-center mt-3">
        <small class="text-muted">
            ...
            <a href="{{ route('register') }}" class="text-decoration-none fw-semibold" style="color: #2d6a4f;">Register
                here</a>
        </small>
    </div>
